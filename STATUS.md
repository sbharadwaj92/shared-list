...
- iOS Info.plist gains scoped `localhost` / `127.0.0.1` ATS exceptions (allow insecure HTTP on those hosts only) so the integration suite can hit Bun directly without going through Caddy + mkcert. Useful for both the CI-future and bare-machine local runs.
- **No CI workflow for the integration suite** — the integration tests run locally before merging anything wire-shape-changing. STATUS log entry below + `ios/README.md` document the convention. First attempt (`.github/workflows/ios-integration.yml` booting Postgres via Colima on macos-15) hit a wall: GitHub hosted macOS runners don't support nested virtualization (no `VZ.framework` / no `HVF`), Colima can't boot its VM, so Docker → Postgres → backend bring-up never starts. Workflow file removed in the same slice rather than left as a permanent red ✗ on every PR. The five fix paths (self-hosted runner, split-runner tunneling, embedded `pglite`, paid Mac CI, leave-as-manual) were enumerated and the user picked "leave as manual"; revisit in Phase 19 polish or once a related task creates the dependency. The unit suite (50→54 iOS tests) + the backend's own Testcontainers tests pin the wire contract on every PR, so this isn't a coverage hole — only the cross-process round-trip is unguarded in CI.
