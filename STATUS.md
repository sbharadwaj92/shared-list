...
**Phase**: Phase 7 IN PROGRESS (started 2026-05-05) — slices A + B merged (PRs #8, #9); slice C.1 code-complete on `phase-07-backend-writes`; ready to open PR
**Next action**: open the slice-C.1 PR (commit + push `phase-07-backend-writes`, then `gh pr create`). After it merges, **slice C.2** lands the iOS `MutationQueueEntry` `@Model` + optimistic-apply path; **slice C.3** lands the drainer + 409→reconcile + real-backend Testcontainers integration test (the cycle slice B deferred). Slice D wraps with tombstone fuzz + `docs/learning/phase-07.md`.

Slice C.1 ships:

- `POST /lists` — idempotent via client UUID v7 + `INSERT ... ON CONFLICT (id) DO NOTHING`. Actor becomes the `owner` member in the same transaction (`insertListWithOwner` repo helper). `201` on first create, `200` on idempotent retry, `409` if the id collides with a tombstoned list.
- `POST /lists/:id/items` — same idempotency story; gated on active membership of the parent list.
- `PATCH /lists/:id` — `If-Match: <ISO8601>` required; conditional update via `eq(updated_at)` CAS pinned with `deleted_at IS NULL`; `200` on success, `409` with `{error, latest: ListDTO}` body on mismatch (PLAN.md L62 picked 409 over RFC 7232's 412 to keep all "your write didn't land" failures on one status).
- `PATCH /items/:id` — `text` / `position` / `checked` patchable individually; same `If-Match` semantics; empty body rejected as 400.
- `DELETE /lists/:id` — owner-only soft-delete; cascades to items in the same transaction; each affected item's `updated_at` bumped via `date_trunc('milliseconds', now())` so the `?since=` items feed surfaces every tombstone; already-tombstoned items left untouched.
- `DELETE /items/:id` — soft-delete; requires active membership of the parent list.

All endpoints behind `requireAuth`. New repo helpers (`insertListWithOwner`, `conditionalUpdateListName`, `softDeleteListCascade`, `insertItem`, `conditionalUpdateItem`, `softDeleteItem`) co-located with their resource. DTO converters extracted to `sync/dto.ts` so write responses share the wire shape with the read feed. `backend/docs/sync.md` Status table updated; new "Write side (slice C.1)" section documents idempotent POST, If-Match conditional update, cascade soft-delete, plus per-endpoint specs and how write responses interact with the `?since=` feed.

Tests: 22 new HTTP integration tests (11 lists + 11 items) pin idempotent-retry shape, If-Match 200/409 with `latest`, owner-only DELETE + items-cascade with `updated_at` bump, cross-user 403, soft-deleted-id collision 409, empty-PATCH 400, missing-If-Match 400. Backend total: 70 → 92 tests, all green.
**Blockers**: none
...
- [x] iOS: SwiftData `@Model` types + `ModelContainer` *(`UserModel`, `ListModel`, `ItemModel`, `MemberModel`, `SyncCursor`; `MutationQueueEntry` deferred to slice C.2)*
- [x] iOS: `NetworkMonitor` (`NWPathMonitor`-backed `@Observable`)
- [ ] iOS: `SyncEngine` with mutation queue, drainer, reconciliation, LWW *(slice B landed read-only reconciliation only — mutation queue/drainer/LWW land in C.2/C.3)*
