...
- A real-backend integration test (the cycle slice B deferred) using Testcontainers Postgres + the actual Hono app: spin up the backend in-process, point the iOS APIClient at it, call Mutator → drain → reconcile, assert the local + backend states converge for the offline-mutate / reconnect / reconcile cycle.
