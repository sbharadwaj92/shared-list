...
**Phase**: **Phase 7 DONE 2026-05-05** (slices A + B + C.1 + C.2 + C.3 + D merged via PRs #8, #9, #10, #11, #12, #13); ready to start Phase 8
**Next action**: begin **Phase 8** — Android persistence + sync engine. Branch off main as `phase-08-android-sync`. Mirror the iOS design layer-by-layer:

- Room entities matching the SwiftData `@Model` types (`UserModel`, `ListModel`, `ItemModel`, `MemberModel`, `SyncCursor`, `MutationQueueEntry`).
- `ConnectivityManager` + `NetworkCallback` `NetworkMonitor` mirroring iOS's `NWPathMonitor`-backed `@Observable`.
- `SyncEngine` mirroring iOS slice B (read-side reconciler with the same lists → items → members ordering, the same self-revocation sweep).
- `Mutator` mirroring iOS slice C.2 (atomic local-apply + queue-append in one Room transaction; LWW-friendly pre-stamp; cascade soft-delete; 3-state `OptionalChange` for the wire-side `checked` field).
- `Drainer` mirroring iOS slice C.3 (serial drain, 409 → reconcile + retry-once via the existing LWW upsert, status code handling for 2xx/401/404/403/5xx/network).
- A `JSONCoders`-equivalent on Kotlin: kotlinx.serialization with an explicit fractional-millis ISO8601 instant serializer, NOT the default. The slice D bug was iOS-only by chance; Kotlin's defaults need the same scrutiny.
- JUnit unit tests covering the same scenarios as iOS (Mutator atomicity + If-Match prior-cursor, Drainer 409 reconcile + 5xx requeue cap, SyncFuzz hostile cycles).
- Env-gated integration tests against the same locally-running backend, same convention as iOS (`BACKEND_URL=…` before the Gradle test task; skip silently otherwise; document local invocation in `android/README.md`).
- `docs/learning/phase-08.md` — teach-back focused on **what changed** vs Phase 7 (Room transaction shape vs SwiftData's `context.save()`, kotlinx.serialization vs Foundation Codable, `StateFlow` collection in Compose vs `@Observable` in SwiftUI). Explicitly NOT a re-derivation; PLAN.md L382 is clear that the protocol is frozen and Android consumes it cleanly.

Phase 8 is mirror-work, not new design. The interesting parts are where the platforms genuinely differ; everything else should port one-to-one.
**Blockers**: none
