...
**Phase**: Phase 7 IN PROGRESS (started 2026-05-05) — slices A + B + C.1 + C.2 + C.3 merged (PRs #8, #9, #10, #11, #12); slice D code-complete on `phase-07-wrap`; ready to open PR
**Next action**: open the slice-D PR. After it merges, **Phase 7 hits DONE 2026-05-05** and Phase 8 (Android persistence + sync engine) opens.

Slice D ships:

- 4 new `SyncFuzzTests` covering hostile scenarios from PLAN.md L231 — rapid create-delete-create (queue ordering + cascade interaction), simultaneous-edits-chained-If-Match (LWW correctness on the wire across rapid rename calls), edit-on-server-deleted (404 on PATCH = idempotent shape, drainer removes queue row, reconciler sweeps), and list-delete-while-items-have-pending-mutations (the load-bearing tombstone scenario — local cascade tombstones items immediately + server cascades on its end).
- New `JSONCoders` module exposing `makeEncoder()` / `makeDecoder()` that use a shared `ISO8601DateFormatter` with `.withFractionalSeconds`. Replaces every `dateEncodingStrategy = .iso8601` and `Date.formatted(.iso8601...)` call in APIClient / Mutator / Drainer / SyncEngine / SyncEngine cursor formatter. **Real wire-protocol bug fixed**: Foundation's default `.iso8601` JSON strategy is second-precision (drops millis on encode), AND `Date.formatted(.iso8601...)` silently TRUNCATES the fractional component instead of rounding (so `9000.001999...` renders as `02:30:00.001` instead of `02:30:00.002`). Both bugs would have caused 409 storms in production once two Mutator calls happened within a millisecond. Caught by the simultaneous-edits fuzz test.
- New `.github/workflows/ios-integration.yml` — the CI job deferred from slice C.3. macOS-15 runner boots Postgres via Colima + docker compose, runs migrations, starts `bun run dev` on plain HTTP at `localhost:3000` (no Caddy in CI — the iOS Info.plist gains scoped `localhost` / `127.0.0.1` ATS exceptions that allow insecure HTTP on those hosts only), sets `BACKEND_URL=http://localhost:3000`, runs only the `DrainerIntegration` test bundle. Backend log surfaces on failure. Picked HTTP+localhost over installing mkcert root CA on the runner because loopback traffic never leaves the runner and the alternative was significantly more setup for one integration test. **Workflow is `continue-on-error: true`** because the GitHub `macos-15` hosted runner does not support nested virtualization (VZ.framework) — Colima can't boot its VM, so Docker → Postgres → backend bring-up fails. The workflow is left in place to surface "the integration job ran" without blocking PRs, and as a starting point for a future Phase 19 polish (self-hosted runner, split runners with tunneling, or embedded-pglite for CI). The integration tests still run locally per `ios/README.md`.
- `docs/learning/phase-07.md` — the central learning artifact for the project. ~1500-word teach-back covering the cursor model, the read feed's tombstone-flow rationale, the write side's idempotent POST + If-Match conditional update, the iOS sync engine's read-side reconciler + mutation queue + drainer + 409→reconcile→retry-once, and LWW's data-loss trade-off. 4 rejected alternatives: server-vouched cursor, 409-vs-412 status, UUID v4 (vs v7) shortcut, env-gated tests vs Process-spawned Testcontainers, mutation queue persisted in SwiftData. 3 break-it scenarios: the millisecond-precision bug surfaced by the fuzz test (above), the 5xx-only infinite-loop bug surfaced by the Drainer unit test, the mid-cycle server-deletes-while-client-edits trade-off (don't duplicate sweep work between drainer + reconciler).
- `ios/README.md` updated with the local-HTTP option for running the integration tests against a backend without going through Caddy + mkcert.

Tests: 4 new `SyncFuzzTests` (50 → 54 iOS tests), all green; xcodebuild clean for iPhone 17 Pro Max sim.
...
2026-05-05 — Phase 7 slice D code-complete: the wrap. 4 new `SyncFuzzTests` covering hostile scenarios (rapid create-delete-create, simultaneous-edits-chained-If-Match, edit-on-server-deleted, list-delete-while-items-have-pending-mutations). The simultaneous-edits scenario surfaced a real wire-protocol bug: Foundation's `JSONEncoder.dateEncodingStrategy = .iso8601` is second-precision (drops millis on encode) AND `Date.formatted(.iso8601...)` silently TRUNCATES the fractional component (so `9000.001999...` renders as `02:30:00.001` instead of `02:30:00.002`). Two distinct bugs in two layers; both would have caused 409 storms in production once two Mutator calls happened within a millisecond. Fixed via a new `JSONCoders` module exposing `makeEncoder()` / `makeDecoder()` that share an `ISO8601DateFormatter` with `.withFractionalSeconds`; replaced every callsite in APIClient / Mutator / Drainer / SyncEngine. New `.github/workflows/ios-integration.yml` was authored to boot Postgres via Colima + docker compose on macOS-15, run migrations, start `bun run dev` on plain HTTP at `localhost:3000`, set `BACKEND_URL`, and run only the `DrainerIntegration` test bundle. **Hit a wall**: GitHub's `macos-15` hosted runner doesn't support nested virtualization, so Colima can't boot its VM and Docker → Postgres → backend bring-up fails. Marked the workflow `continue-on-error: true` and documented the limitation + future fix paths (self-hosted runner / split runners with tunneling / embedded-pglite for CI) inline. Tests still run locally per `ios/README.md`. The unit suite + the backend's own Testcontainers integration tests pin the wire contract, so this isn't a coverage hole — just an inability to gate PRs on a real-network round-trip in CI for now. Required `localhost` + `127.0.0.1` ATS exceptions in iOS Info.plist allowing insecure HTTP on those hosts only — loopback traffic never leaves the runner so no real security weakening when the workflow does eventually run; the alternative (install mkcert root CA + Caddy on the runner) was significantly more setup. `docs/learning/phase-07.md` written: ~1500-word teach-back covering cursor model + read-feed + write side + iOS sync engine + LWW; 4 rejected alternatives (server-vouched cursor, 409-vs-412, UUID v4 shortcut, env-gated tests, SwiftData mutation queue); 3 break-it scenarios. iOS test count 50 → 54, all green.
