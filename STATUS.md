...
**Phase**: Phase 7 IN PROGRESS (started 2026-05-05) — slices A + B + C.1 + C.2 merged (PRs #8, #9, #10, #11); ready to start slice C.3 (drainer + 409→reconcile + real-backend test)
**Next action**: begin **slice C.3** — the drainer. Branch off main as `phase-07-ios-drainer`. Scope:

- A `Drainer` class on `@MainActor` (or with a background `ModelContext` if profiling demands) that polls the `MutationQueueEntry` rows in `createdAt` order, decodes each `payload` JSON via `MutationOpType` discrimination, builds the right HTTP request against the slice-C.1 backend, and on 2xx removes the row.
- Sequencing: only one in-flight request at a time per slice (we can parallelize later, but a naive serial drain is correct and keeps the merge logic simple). Pick up `pending` rows, flip them to `inFlight` while the request is open, delete on success, flip to `failed` on a permanent error.
- Drainer triggers: `NetworkMonitor` going online, app foregrounding, post-Mutator-call (so the local apply + drain both happen in one user action when online).
- 409 → reconcile path: when a `PATCH` returns 409 with the `latest: …DTO` body, run a targeted reconcile for that resource (single-row upsert via the existing `SyncEngine.upsert*` helpers) THEN retry the PATCH with the new `updatedAt` as the next `If-Match`. Cap at one retry — repeated 409s mean another device is rapidly editing the same row, which is a UX problem we surface rather than spin on.
- Other error handling: 401 lets the existing `APIClient` single-flight refresh handle it (already wired); 5xx and network errors increment `retryCount` and stay `pending` for the next tick (slice D may add jittered backoff); 4xx other than 401/409 mark `failed` with `lastError` populated.
- A real-backend integration test (the cycle slice B deferred) — env-gated. The test reads `BACKEND_URL` from the environment; if unset, it skips. If set, it points the iOS `APIClient` at that URL and runs the offline-mutate → reconnect → drain → reconcile cycle, asserting local + backend convergence. Backend lifecycle is the runner's job: `bun run dev` in another terminal locally, a CI step in workflow. Picked over a Process-based "Swift boots Bun + Testcontainers in setUp" approach because (a) PLAN.md L380 only requires "real backend", not "test self-bootstraps the backend"; (b) keeps the test code small and avoids fragile cross-process management in Swift; (c) matches the shape of how real iOS dev already runs locally — backend runs continuously, app talks to it. `ios/README.md` will document the local invocation and the backend CI workflow gains a job that boots Postgres + Bun and runs the test against it.

After C.3 merges, **slice D** wraps Phase 7 with tombstone-fuzz tests + `docs/learning/phase-07.md`.
