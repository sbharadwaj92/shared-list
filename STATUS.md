...
**Phase**: Phase 7 IN PROGRESS (started 2026-05-05) — slices A + B + C.1 merged (PRs #8, #9, #10); ready to start slice C.2 (iOS mutation queue)
**Next action**: begin **slice C.2** — iOS optimistic-apply + persistent mutation queue. Branch off main as `phase-07-ios-mutation-queue`. Scope:

- `MutationQueueEntry` `@Model` (id, opType, targetId, payload JSON, createdAt, status, retryCount, lastError) added to the SwiftData container.
- A small `Mutator` API (`createList`, `renameList`, `deleteList`, `createItem`, `patchItem`, `deleteItem`) that does two things on every call: (1) applies the change to the local SwiftData store immediately for instant UI, (2) appends a `MutationQueueEntry` row to durably record the intent. No network here — slice C.3 owns the drainer.
- The optimistic-apply pre-stamps the local `updatedAt` to "now" so subsequent reads + the eventual server response can LWW-resolve cleanly. Items get an integer-position helper that picks "max(positions)+1024" for new items.
- Soft-delete locally on `delete*` paths so feature views (which filter `deletedAt == nil`) hide the row immediately, with the server `DELETE` queued for the drainer.
- New Swift Testing suite drives the Mutator against an in-memory `ModelContainer`, asserts the local row + queue entry shape after each op, and locks down the LWW-friendly local timestamp pre-stamp.

After C.2 merges, **slice C.3** lands the drainer, the `If-Match` 409 → reconcile path, and a real-backend Testcontainers integration test (the cycle slice B deferred). **Slice D** wraps with tombstone fuzz + `docs/learning/phase-07.md`.
