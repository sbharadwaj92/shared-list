...
**Phase**: Phase 7 IN PROGRESS (started 2026-05-05) — slices A + B + C.1 + C.2 + C.3 merged (PRs #8, #9, #10, #11, #12); ready to start slice D (the wrap)
**Next action**: begin **slice D** — Phase 7 wrap-up. Branch off main as `phase-07-wrap`. Scope:

- **Tombstone-fuzz tests**: a Swift Testing harness (and likely a backend-side Bun fuzz too) that drives rapid create-delete-create / simultaneous-edits / edits-on-deleted / lists-deleted-while-editing scenarios across Mutator + Drainer + SyncEngine and asserts convergence to a coherent end state. PLAN.md L231 lists the scenarios; we don't need 100% — pick the 4-6 that exercise the most LWW + tombstone interactions and prove the cycle holds.
- **Env-gated integration test CI workflow** (deferred from C.3): a new `.github/workflows/ios-integration.yml` (or a job inside `ios.yml`) that on macOS-15 boots Postgres via docker-compose, starts the backend with `bun run dev` in the background, sets `BACKEND_URL` to the right hostname, then runs the `DrainerIntegration` test bundle. Resolve the mkcert hostname via `/etc/hosts` (`echo "127.0.0.1 Santoshs-MacBook-Pro-48.local" | sudo tee -a /etc/hosts`) and either (a) install the mkcert root CA into the runner's keychain, or (b) skip TLS by pointing the test at the bare Bun port + adding an HTTP-allowed ATS exception for CI. Pick whichever proves cheapest — the PR comment thread on C.3 captured the trade-off.
- **`docs/learning/phase-07.md`**: the per-phase learning artifact per `LEARNING_PROTOCOL.md` — teach-back, rejected alternatives, optional break-it log. Phase 7 is THE central learning artifact for this project; spend the time on it.

Once slice D merges, Phase 7 hits DONE 2026-05-?? and Phase 8 (Android persistence + sync engine) opens.
