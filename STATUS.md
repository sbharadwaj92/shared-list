...
**Phase**: **Phase 10 code-complete on `phase-10-ws-push` branch** (commits 4d1c25b + 2b978d6 + 0f8c99f); ready for PR + CI
**Next action**: open PR for Phase 10, verify CI green, then merge. After merge: begin Phase 11 (iOS WebSocket + push receiver). Phase 10 delivered three slices: A — WS infrastructure (`/ws` upgrade with JWT-in-query-string auth, typed messages subscribe/unsubscribe/ping → ack/pong/event/error, `EventPublisher` seam with Bun pub/sub backing, mutation routes publish on every successful state change); B — `POST /devices` (idempotent registration with `ON CONFLICT (token) DO UPDATE` for the device-handover case where APNs/FCM hand the same token to a new user on the same physical device); C — push module (APNs HTTP/2 sender with cached ES256 JWT from `.p8`, FCM HTTP v1 sender with cached OAuth2 access token from service-account JSON, pg-boss queue with retry/backoff, worker dispatch with PushResult-to-queue-action mapping). Push is off-by-default in dev (`PUSH_ENABLED=false`) so a fresh clone boots without any Apple/Firebase creds. Live wscat-equivalent verification passed end-to-end against the dev backend (signup → create list → WS connect → subscribe → ack → HTTP item create → event arrives with correct entity/action/id/listId → unsubscribe → ack → ping → pong). 191/191 backend tests pass; biome + tsc clean.

The mutation-to-push fan-out (one push per non-actor member when a shared list changes) is intentionally NOT wired here — that's Phase 16 because the actor-exclusion + "shared-list-only" gating is integral to that wiring per PLAN.md L411. Phase 10 ships infrastructure; Phase 11/12 ship receivers; Phase 16 ships the consumer.

Phase 9 ground rules carry forward + are extended: the WS event payload shape (entity + action + id + listId + at) needs the same cross-platform discipline as the `?since=` wire contract. Phase 11's iOS WS receiver and Phase 12's Android WS receiver must consume the same event JSON without per-platform special-casing; any change to the event shape touches `backend/docs/sync.md` + the `messages.ts` Zod schemas + Phase 11's iOS receiver + Phase 12's Android receiver in lockstep.
**Blockers**: none
...
#### Phase 10 — Backend WebSocket server + push infrastructure — IN PROGRESS (code-complete, awaiting PR + CI + merge)
- [x] Bun-native WS handler under `/ws`
- [x] Per-user socket map + per-socket subscription set *(per-socket Set on `ws.data.subscriptions` + Bun's native pub/sub topic index for fan-out; no separate per-user Map needed at this scale)*
- [x] Typed messages: subscribe / unsubscribe / event *(plus ping/pong/ack/error rounding out the protocol)*
- [x] WS auth via `?token=<jwt>` query string at upgrade
- [x] Mutation endpoints publish events to subscribed sockets *(`EventPublisher` seam; `BunEventPublisher` for production, `InMemoryEventPublisher` for tests)*
- [x] APNs sandbox HTTP/2 with `.p8` key *(ES256 JWT cached ~55min; reason-based PushResult mapping for Unregistered / ExpiredProviderToken / TooManyRequests / ServiceUnavailable / BadTopic)*
- [x] FCM HTTP v1 API with service account JSON, `priority: high` *(uppercase HIGH per v1 contract; OAuth2 access token cached ~1h)*
- [x] pg-boss job queue for delivery *(5 retries with exponential backoff, base 30s)*
- [x] `POST /devices` registers (user, platform, token) *(idempotent on client-supplied UUID v7; ON CONFLICT (token) DO UPDATE handles the device-handover case)*
- [x] WS works via `wscat` *(used a Bun WebSocket script equivalent; live-verified end-to-end against the dev backend — signup → connect → subscribe → ack → HTTP mutation → event → unsubscribe → ack → ping → pong all pass)*
- [ ] APNs sandbox + FCM dev pushes verified via Apple/Google consoles *(deferred — requires .p8 + service account JSON from Apple Developer / Firebase; senders + protocol tested in unit + integration tests against mocked HTTPS responses including every documented error class; live console verification will happen alongside Phase 11/12 receiver wiring when there's actually a real device to send to)*
- [x] `docs/learning/phase-10.md` written
...
2026-05-12 — Phase 10 code-complete on `phase-10-ws-push` branch in three slices. **Slice A** (4d1c25b) — WS infrastructure under new `features/realtime/`: `/ws` upgrade handler with JWT-via-query-string auth (`?token=<jwt>`), typed message protocol (subscribe / unsubscribe / ping → ack / pong / event / error) via Zod discriminated unions, `SocketData` attached at `server.upgrade(req, {data})` so per-socket userId/connectionId/subscriptions Set rides with the connection, dispatcher re-checks `activeMembership` on every subscribe (JWT proves identity, membership proves entitlement), unsubscribe NOT gated so revoked users can drop stale subscriptions. `EventPublisher` seam — `BunEventPublisher` wraps `server.publish()` via Bun's native uWebSockets pub/sub for fan-out, `InMemoryEventPublisher` records published events synchronously for fast route tests. Lists/items mutation routes (POST/PATCH/DELETE) take a publisher arg and emit `{entity, action, id, listId, at}` on every successful state change; idempotent POST retry that returns 200 does NOT emit (no real state change). 40 new tests across 4 files (messages parse, ws dispatcher against fake socket + real DB membership, full Bun.serve + WebSocket integration, mutation→event wiring). **Slice B** (2b978d6) — `POST /devices` feature: schema validation, repo with `ON CONFLICT (token) DO UPDATE` for device-handover (token registered to Alice then later by Bob on the same physical device MOVES the row to Bob; otherwise Alice keeps receiving Bob's pushes), 12 new tests across 2 files including the load-bearing user-handover via HTTP. **Slice C** (0f8c99f) — push module under new `features/push/`: APNs HTTP/2 sender minting cached ES256 JWT from `.p8` via jose, reason-based PushResult mapping for Unregistered / BadDeviceToken / ExpiredProviderToken / TooManyRequests / ServiceUnavailable / BadTopic (status alone is insufficient — 400 covers both permanent BadDeviceToken and retryable ExpiredProviderToken); FCM HTTP v1 sender with two-step auth (RS256 JWT → OAuth2 access token → FCM send), `android.priority: 'HIGH'` (uppercase, v1 contract), error-code mapping for UNREGISTERED / UNAVAILABLE / INVALID_ARGUMENT / QUOTA_EXCEEDED; pg-boss queue wrapper with retry config (5 attempts, exponential, base 30s) + `InMemoryPushQueue` for tests; worker dispatcher translating PushResult to queue-action (retry → throw, unregistered → onUnregistered hook + return, invalid → log + return); `PushService` facade with `Disabled` + `Enabled` impls — disabled is the default so dev clones boot without Apple/Firebase creds. Config validation: PUSH_ENABLED=true requires every cred field via superRefine. Wired through index.ts boot path; smoke-tested boot with `PUSH_ENABLED=false` and confirmed graceful-shutdown. 47 new tests across 5 files (apns + fcm with stubbed fetch, queue contract, worker dispatch, service fan-out). Push fan-out from mutation routes intentionally NOT wired — that's Phase 16's job per PLAN.md L411 (actor-exclusion + shared-list-only gating). Backend test count: 92 → 191. Live wscat-equivalent verification passed end-to-end against `bun run dev` on `localhost:3000`: signup → create list → WS connect → subscribe → ack → HTTP item create → event arrives with correct entity/action/id/listId → unsubscribe → ack → ping → pong. POST /devices live-verified too. Live APNs/FCM console verification deferred to Phase 11/12 alongside real device wiring — senders + protocol fully unit + integration tested against mocked HTTPS responses for every documented error class. `docs/learning/phase-10.md` written: ~1500-word teach-back covering the WS-as-freshness-signal contract, the EventPublisher seam, the device-handover semantics, the APNs/FCM protocol shapes, and the disabled-by-default ergonomics; 5 rejected alternatives (query-string vs cookie vs header WS auth, event-with-body vs freshness-only, EventPublisher seam vs direct server.publish, single vs composite uniqueness on device_tokens, pg-boss vs hand-rolled SKIP LOCKED queue); 5 break-it scenarios documented (server-crash mid-WS, subscribe-after-revoke, malformed JSON, 100-list subscription stress, malformed APNS_PRIVATE_KEY). `backend/docs/sync.md` gains a "WebSocket layer (Phase 10)" section + a "Device registration (Phase 10)" section; status table refreshed through Phase 10. `backend/README.md` gains a "WebSocket + push (Phase 10)" section covering enable-push env vars and "if pushes never arrive" diagnostics. PR ready.
